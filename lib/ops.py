...
def preprocessadv(image):
    with tf.name_scope("preprocessadv"):
        return tf.identity(image)


def deprocessadv(image):
    with tf.name_scope("deprocessadv"):
        return tf.identity(image)
...
def random_flip(input, decision):
    ...
    # tf.image.flip_left_right zuoyou fanzhuang
...
def coupute_accuracy(inputs,height,width):
    inputs = preprocess_image(inputs,height,width)
    with slim.arg_scope(inception.inception_v3_arg_scope()):
        logits, _ = inception.inception_v3(inputs,num_classes=1001,is_training=False)
    pass
